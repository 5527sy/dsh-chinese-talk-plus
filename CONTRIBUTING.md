...
pnpm install
pnpm run check
python -m py_compile bridge/record_sink.py bridge/voice_clone.py bridge/__init__.py
```
...
Voice-cloning model weights (Qwen3-TTS) are self-provisioned and git-ignored, like the FunASR models — point `QWEN_TTS_DIR` at a local copy when developing.
